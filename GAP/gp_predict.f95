...
module gp_predict_module
   ...
   integer, parameter, public :: PP_Q = 2
...
!!$omp end parallel
   ...
   pure function covariancePP(r,q,d)
      ...
      elseif( r < 0.0_dp ) then
         ...
         if( q == 0 ) then
            covariancePP = (1.0_dp - r)**j
         elseif( q == 1 ) then
            covariancePP = (1.0_dp - r)**(j+1.0_dp) * ( (j+1.0_dp)*r + 1.0_dp )
         elseif( q == 2) then 
            covariancePP = (1.0_dp - r)**(j+2.0_dp) * ( (j**2 + 4.0_dp*j + 3.0_dp) * r**2 + (3.0_dp*j+6.0_dp)*r + 3.0_dp ) / 3.0_dp
         elseif( q == 3) then 
            covariancePP = (1.0_dp - r)**(j+3.0_dp) * &
   ...
   pure function grad_covariancePP(r,q,d)
      ...
      elseif( r < 0.0_dp ) then
         ...
         if( q == 0 ) then
            grad_covariancePP = - j * (1.0_dp - r)**(j-1.0_dp)
         elseif( q == 1 ) then
            grad_covariancePP = - (j+1.0_dp) * (1.0_dp - r)**j * ( (j+1.0_dp)*r + 1.0_dp ) + &
            (1.0_dp - r)**(j+1.0_dp) * (j+1.0_dp)
         elseif( q == 2) then 
            grad_covariancePP = - (j+2.0_dp) * (1.0_dp - r)**(j+1.0_dp) * ( (j**2 + 4.0_dp*j + 3.0_dp) * r**2 + (3.0_dp*j+6.0_dp)*r + 3.0_dp ) / 3.0_dp + &
            (1.0_dp - r)**(j+2.0_dp) * ( 2.0_dp * (j**2 + 4.0_dp*j + 3.0_dp) * r + (3.0_dp*j+6.0_dp) ) / 3.0_dp
         elseif( q == 3) then 
            grad_covariancePP = -(j+3.0_dp) * (1.0_dp - r)**(j+2.0_dp) * &
            ( (j**3 + 9.0_dp*j**2 + 23.0_dp*j + 15.0_dp)*r**3 + &
              (6.0_dp * j**2 + 36.0_dp*j + 45.0_dp) * r**2 + &
              (15.0_dp*j+45.0_dp)*r + 15.0_dp ) / 15.0_dp + &
            (1.0_dp - r)**(j+3.0_dp) * &
...
!   endfunction gpCovariance_soap_Calc
   ...
   function gpCoordinates_Predict( this, xStar, gradPredict, variance_estimate, do_variance_estimate, grad_variance_estimate, error )
      ...
      logical, intent(in) :: do_variance_estimate
...
!         end do
         ...
         k(:) = this%delta**2 * covariance_x_xStars(:)**this%zeta
         if(present(gradPredict)) then
            allocate(alpha_scaled(size(this%alpha)))
            alpha_scaled(:) = this%alpha(:) * this%delta**2 * this%zeta * covariance_x_xStars(:)**(this%zeta-1.0_dp)
         endif
